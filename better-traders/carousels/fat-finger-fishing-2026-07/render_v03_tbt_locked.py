...
root = Path('/Users/bettybot/clawd/buffer-media-assets/better-traders/carousels/fat-finger-fishing-2026-07')
VERSION = 'v03'
...
def add_gradient(im):
    ...
    for y in range(h):
        for x in range(w):
            left = max(0, 1 - x / 730)
            top = max(0, 1 - y / 580)
            bottom = max(0, (y - 850) / 240)
            a = min(248, int(218 * left + 60 * top + 115 * bottom))
            pix[x, y] = (0, 4, 10, a)
...
bases = {
    1: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142135_7210ab1c.png'),
    2: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142244_b0d30164.png'),
    3: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142353_4374080c.png'),
    4: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142645_d0b6deb9.png'),
    5: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142802_dfa18078.png'),
    6: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_142956_fbb5a20b.png'),
    7: Path('/Users/bettybot/.hermes/cache/images/openai_codex_gpt-image-2-medium_20260707_143104_73d20d84.png'),
}

cfg = {
    1: ('FAT FINGER\nFISHING\nIS REAL', 'A flash wick can hand\nyou an entry most traders\nnever thought possible.', 70, 152, 58, 27, 700),
    2: ('The order waits\nway below price', 'This is not chasing.\nYou park the buy where\na surprise dump would\nbe an opportunity.', 70, 160, 55, 27, 705),
    3: ('Then the wick\ngets ridiculous', 'One fast candle.\nA thin book.\nA panic dump.\nYour low bid is exactly\nwhere the market stabs.', 70, 150, 55, 26, 705),
    4: ('That fill is\nthe whole edge', 'If it hits, your entry\nis massively below where\neveryone was watching.', 70, 164, 58, 27, 700),
    5: ('Now the bounce\ncan pay fast', 'When price snaps back,\na tiny mistake from the\nmarket can become a\nclean AB trade.', 70, 162, 56, 27, 705),
    6: ('Low risk.\nBig asymmetry.', 'You risk only what\nyou planned. The upside\ncomes from getting filled\nat a stupid-low wick.', 70, 158, 59, 27, 705),
    7: ('Exciting,\nbut planned', 'Fat Finger Fishing is\nAB Trading patience:\nlow bids, surprise fills,\nand a clear plan for\nthe rebound.', 70, 146, 58, 25, 715),
}
...
for n, (title, body, x, y, title_size, body_size, maxw) in cfg.items():
    ...
    dest = out / f'tbt-fat-finger-fishing-slide-{n:02d}-{VERSION}-review.png'
...
contact = out / f'tbt-fat-finger-fishing-contact-sheet-{VERSION}-review.png'
...
manifest = {
    'title': 'Fat Finger Fishing Is Real',
    'status': 'REDO REVIEW READY',
    'review_only': True,
    'scheduled': False,
    ...
    'copy_gate': 'Aaron correction incorporated: AB Fat Finger Fishing is a low-risk strategy placing buy orders far below current price for flash-wick low fills, very low entries, and rebound-profit potential; overlay stays educational and avoids exact entry/exit advice.',
    ...
    'qa': {
        ...
        'visuals': 'Marcus remains the protagonist; shows deep waiting bids, flash-wick fill, massive low-entry, rebound management, and excited/profitable AB strategy framing; no phishing/security metaphor drift',
        'compliance': 'AB Trading education only; no exact entries, tickers, prices, P&L, broker UI, buy/sell commands, or guaranteed fill/profit claims',
