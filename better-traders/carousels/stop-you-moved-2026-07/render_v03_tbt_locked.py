...
for n, (title, body, x, y, title_size, body_size, maxw) in cfg.items():
    ...
    if n == 6:
        ...
        # not AI scribbles. Draw it ourselves so it stays readable.
        journal = ImageDraw.Draw(im)
        sentence = 'My invalidation is set before I enter.'
        x0, y0 = 575, 548
        journal.text((x0 + 2, y0 + 2), sentence, font=font(30, 'reg'), fill=(248, 235, 204, 110))
        journal.text((x0, y0), sentence, font=font(30, 'reg'), fill=(42, 30, 20, 245))
        journal.line((x0, y0 + 40, x0 + 495, y0 + 40), fill=(38, 26, 17, 235), width=3)
    if n in (1, 7):
        paste_logo(im)
...
manifest = {
    ...
    'copy_gate': 'Claude Worker creative-director feedback incorporated before rerender; v03 applies Aaron note to redo slides 04 and 06 only: slide 04 uses a new over-shoulder decision angle, slide 06 has a real underlined journal sentence instead of AI scribbles.',
    ...
    'qa': {
        ...
        'visuals': 'Dani remains the protagonist; v03 keeps v02 approved character-first direction while replacing slide 04 with a distinct over-shoulder stop-line angle and replacing slide 06 with a clean journal page containing the legible underlined sentence: My invalidation is set before I enter.',
